...
import DesktopSidebarFilters, { defaultFilters } from './desktop-sidebar-filters'
import { getEventsClient, getFeaturedEventsClient, transformEventToFrontend } from '@/lib/events-client'
...
const isThisWeek = (dateString: string) => {
  ...
  const endOfWeek = new Date(today.setDate(today.getDate() - today.getDay() + 6)); // Saturday (after today is modified)
...
type Event = {
  ...
  img: string;
  href?: string;
  date?: string; // Make sure this matches - no null allowed
  language: string;
  ageLimit: string;
  eventType: string;
}
...
export default function DesktopMainLayout({ events: propEvents, showArtistNearYou = true }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        console.log('Fetched events from database:', allEvents.length, 'events')
  ...
  const filteredEvents = useMemo(() => {
    // Map tab id → actual event_type values in the DB
    const categoryToEventType: Record<string, string[]> = {
      'campus': ['campus', 'campus_event'],
      'events': ['event', 'events'],
      'movies': ['movie', 'movies'],
      'sports': ['sports', 'sport'],
    };
    ...
    return events.filter(event => {
      ...
      if (selectedCategory !== 'all') {
        const allowed = categoryToEventType[selectedCategory] || [selectedCategory];
        if (!allowed.includes((event.eventType || '').toLowerCase())) {
          return false;
        }
      }
          ...
          switch (sectionId) {
            ...
            case 'price':
              ...
              for (const option of selectedOptions) {
                ...
                const isEventFree = event.price === 'FREE' || event.price === '0' || event.price.toLowerCase() === 'free';
                const eventPrice = isEventFree ? 0 : parseFloat(event.price.replace(/[^0-9.]/g, '')) || 0;

                if (option === 'free' && isEventFree) {
                  match = true;
                  break;
                }
                if (option === 'under-1000' && !isEventFree && eventPrice > 0 && eventPrice < 1000) {
                  match = true;
                  break;
                }
                if (option === 'under-2000' && !isEventFree && eventPrice >= 1000 && eventPrice < 2000) {
                  match = true;
                  break;
                }
                if (option === 'above-2000' && !isEventFree && eventPrice >= 2000) {
                  match = true;
                  break;
                }
              }
              break;
            case 'date':
              if (event.date) { // Only apply date filter if event has a date
                for (const option of selectedOptions) {
  ...
  const transformedFeaturedEvents = useMemo(() => {
    return featuredEvents.map(event => {
      ...
      const getPosterImage = () => {
        const portraitPoster = typeof event.portrait_poster === 'string' ? event.portrait_poster : null;
        const landscapePoster = typeof event.landscape_poster === 'string' ? event.landscape_poster : null;
      ...
      return {
        ...
        ageRating: event.age_limit || 'PG13',
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-[73px]">
      {/* Event Hero Section removed per user request from inner pages */}

      {/* Category Tabs removed — navigation is handled by main navbar */}

      {/* Main Content Area with Sidebar */}
      <div className="px-[100px] py-8 min-h-screen">
        <div className="flex gap-8">
          {/* Left Sidebar */}
          <div className="w-fit sticky top-[80px] h-fit">
          ...
          <div className="flex-1">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-[28px] gap-x-[26px]">
                  {filteredEvents.map((event) => (
                    <Link
                      ...
                      className="rounded-xl overflow-hidden hover:cursor-pointer transition-all duration-300 group w-full block"
                    >
                      <div className="w-full aspect-[5/6] relative overflow-hidden rounded-lg">
                        <Image
                          src={event.img}
                          alt={event.name}
                          fill
                          sizes="(max-width: 840px) 130vw, (max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw"
                      ...
                      <div className="mt-3">
                        ...
                        <h3 className="text-[15px] text-black group-hover:text-sky-600 transition-colors line-clamp-2">
                          {event.name}
                        </h3>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sky-600 group-hover:text-sky-700 text-sm font-medium opacity-0 group-hover:opacity-100 transition-all duration-200">
                            Book Now →
                          </span>
                        </div>
                      </div>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-gray-600 mb-4">The database doesn&apos;t have any events yet.</p>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md">
                  <p className="text-sm text-blue-800 mb-2">
                    <strong>To add sample events:</strong>
                  </p>
                  <ol className="text-sm text-blue-700 text-left space-y-1">
                    <li>1. Run: <code className="bg-blue-100 px-1 rounded">supabase start</code></li>
                    <li>2. Run: <code className="bg-blue-100 px-1 rounded">supabase db reset</code></li>
                    <li>3. Run: <code className="bg-blue-100 px-1 rounded">psql -h localhost -p 54322 -U postgres -d postgres -f sample-events.sql</code></li>
                  </ol>
                </div>
              </div>
            )}
