...
// Campus data is centralized in src/lib/campuses
...
export default function CampusPage({ params }: CampusPageProps) {
  ...
  const campusInfo = getCampusById(resolvedParams.id);
    if (campusInfo) {
      ...
      // Campus not found, redirect to campuses list or 404
        ...
        <main className="px-4 pb-[96px]">
          ...
          <section aria-labelledby="all-events" className="mb-6">
            <div className="mb-3 flex items-center gap-3">
              <h2 id="all-events" className="text-base font-medium">All {campus.name} Events</h2>
              <button className="text-sm font-medium text-sky-600" aria-label="See all events">{"See all >"}</button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {events
                ...
                .map((event) => {
                  const slug = `${campus.id}-${event.id}`; // e.g. "iit-delhi-e1"
                  return (
                    <Link key={event.id} href={`/sports/Eventname/${slug}`} className="block">
                      <article className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
                        <div className="h-28 w-full bg-zinc-100">
                          <Image src={event.img || "/placeholder.svg"} alt={event.name} width={300} height={112} className="h-full w-full object-cover" />
                        </div>
                        <div className="p-3">
                          <p className="text-[11px] text-zinc-500">{event.category}</p>
                          <h3 className="mt-0.5 line-clamp-2 text-sm font-medium">{event.name}</h3>
                          <p className="mt-1 text-[13px] font-semibold text-emerald-600">{event.price}</p>
                        </div>
                      </article>
                    </Link>
                  );
