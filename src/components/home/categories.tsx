...
export default function Categories({
  ...
  return (
    <section aria-labelledby="categories" className="mb-6 md:mb-8">
      <div className="mb-3 md:mb-4 flex items-center gap-3">
        ...
        <Link href={getEventsPageUrl()}>
          <button className="text-sm md:text-base font-bold text-sky-600" aria-label={`See all ${title}`}>
            {'See all >'}
          </button>
        </Link>
      </div>
  ...
  />
              ...
              <p className="text-xs md:text-sm font-bold text-zinc-700">{category.title}</p>
            </Link>
          </div>
        ))}
      </div>
